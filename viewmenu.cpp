	...
	while (flag)
	{
		...
		case WM_LBUTTONDOWN: // 鼠标左键按下
			...
			if (m.x >= x && m.x <= x + w && m.y >= y - 210 && m.y <= y - 210 + h)
			{
				...
				ss = "resource/存档/" + std::to_string(cen) + "." + std::to_string(room) + "/1.txt";
			...
			{
				// 如果鼠标点击的位置在 "开始游戏" 按钮的范围内
				ss = "resource/存档/" + std::to_string(cen) + "." + std::to_string(room) + "/2.txt";
			...
			{
				// 如果鼠标点击的位置在 "开始游戏" 按钮的范围内
				ss = "resource/存档/" + std::to_string(cen) + "." + std::to_string(room) + "/3.txt";
				map->saveMap(ss);
				flag = 0;
				break;
			}
			...
			{
				// 如果鼠标点击的位置在 "开始游戏" 按钮的范围内
				ss = "resource/存档/" + std::to_string(cen) + "." + std::to_string(room) + "/4.txt";
				map->saveMap(ss);
				flag = 0;
				break;
			}
...
void startGame(int& cen, int& room)
{
	...
	loadimage(&guard, _T("resource/管家图片(1).png")); // 从文件中加载图片
...
void menu1(int& cen, int& room)
{
	...
	IMAGE background; // 定义一个 IMAGE 对象用于存储背景图片
	loadimage(&background, _T("resource/城堡背景(1).jpg")); // 从文件中加载图片
	putimage(0, 0, &background); // 将图片绘制到窗口上，图片左上角坐标为 (0,0)
